...
Train-Test Split and RMSE:
Splitting the data into training and test sets. Fits a linear regression model with various predictors and interaction terms.
evaluates the model using diagnostics and performance metrics. Model did not improve.
...
```{R}
model_without_influential_pts = lm(Rate ~ log1p(Votes) + Duration + Certificate + Nudity + Violence +
                                   Profanity + Alcohol + Frightening + Action + Adventure +
                                   Thriller + Crime + Drama + Sci.Fi + Comedy + History +
                                   Fantasy + Horror + Mystery + Animation + Family +
                                   Romance + Western + Musical + Biography + Music +
                                   War + Sport + Short + Film.Noir + Documentary + Certificate:Frightening + Horror:Frightening + Alcohol:Profanity:Nudity,
                                   data = imdb_data_cleaned)
...
predictions = predict(model_without_influential_pts, newdata = test_data)

rmse = sqrt(mean((test_data$Rate - predictions)^2))
