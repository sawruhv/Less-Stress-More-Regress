...
initial_model = lm(Rate ~ log(Votes) + Date + Duration + Certificate + Nudity + Violence + Profanity + 
...
The diagnostics for our initial additive model were lukewarm. The `Residuals vs Fitted` showed a slight, but discernible, funnel shape. The `Normal Q-Q` plot showed deviations from the diagonal line at tail end of the lower-quantiles. This indicated that the residuals are not perfectly normally distributed. The Shapiro-Wilk test corroborated this finding, with a p-value of `r diagnostics_result$p_val`, leading to the rejection of the null hypothesis of normality. Notably, the Adjusted R-Squared for this additive model was `r initial_model_summary$adj.r.squared`.
...
Removing influential points did not significantly change our diagnostic plots, and the Adjusted R-Squared of this new model was now `r model_without_influential_pts_summary$adjusted.r.squared`.
...
The Boxcox transformation on the response variable seemed to have a pretty good impact on the diagnostics of our model. 
...
TODO: Create a table of the results of our models. Might want to use some library for proper formatting/embedded plot images.

Below you can find an assessment of the models we tried. The order of the rows in this table match our approach chronologically.

| Model Description | Adjusted R-Squared | Sharp-Wilk P-value | Residuals vs Fitted | Q-Q Plot | 
e.g.
| simple additive   |   0.20             | sw pvalue          | plot image          | plot img |

Based on this, ultimately we landed on XYZ model. This isn't the perfect model, but we feel reasonably confident that this model could we used to predict ratings. 

To illustrate this, we compare our prediction ability from our starting model, to the model we ended with.

# TODO: split data, compare prediction from initial additive model, to the last model. Maybe put results in an easy to read table?
...
# intial model trained, make sure to have the data only be pointing to train_data
...
veni, vedi, not necessarily vici

We 
1. cleaned
2. feature engineered
3. tried various approaches to build a model, transform the model to have better fit

SCURTINIZE our results based from a diagnostic points, but draw attention to the fact that our exploration and iteration did improve our diagnostics.